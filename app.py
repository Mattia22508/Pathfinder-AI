...
app = Flask(__name__)
# La chiave segreta serve a Flask per ricordare gli utenti attivi nelle sessioni
...
# ==========================================
# ROTTE DI NAVIGAZIONE BASE (Frontend)
# ==========================================
@app.route('/')
def home():
...
# ==========================================
# 1. ACCESSO E REGISTRAZIONE (Con Password Criptata)
# ==========================================
...
def auth():
    if request.method == 'POST':
        ...
        password = request.form.get('password')  # Prendiamo la password inserita nel form
        ...
        if len(risposta.data) > 0:
            ...
            # Se ha già un abbonamento attivo, salta il checkout e va in dashboard
            ...
            # CRITTOGRAFIA MILITARE: Generiamo l'hash sicuro della password prima di salvarla
            password_criptata = generate_password_hash(password) if password else None
            
            nuovo_utente = {
                'nome_completo': username,
                'email': email,
                'password_hash': password_criptata,  # Salviamo la password blindata nel DB
                'metodo_accesso': 'email',
                'piano_abbonamento': 'gratuito'
            }
            # Inseriamo i dati e ci facciamo restituire l'ID generato in automatico
            ...
            # Mandiamo il nuovo utente appena iscritto a pagare (o inserire il codice prof)
            return redirect(url_for('checkout'))
        
    return render_template('auth.html')

# ==========================================
# 2. CHECKOUT E LOGICA DEI CODICI PROMO (VIP Bypass)
# ==========================================
@app.route('/checkout', methods=['GET', 'POST'])
def checkout():
    # Sicurezza: devi essere loggato per poter accedere al checkout
    ...
    if request.method == 'POST':
        ...
        importo_pagato = 0.0
        giorni_validita = 30  # Default di un abbonamento standard mensile
        piano_assegnato = piano_scelto
        ultime_cifre = '4242'  # Simulazione carta di credito di test
        ...
        if codice_inserito:
            ...
            if len(controllo_promo.data) > 0:
                # CODICE VALIDO! (Applichiamo i superpoteri del codice)
                dati_codice = controllo_promo.data[0]
                piano_assegnato = dati_codice['tipo_piano']
                giorni_validita = dati_codice['giorni_validita']
                importo_pagato = 0.0  # Accesso gratuito!
                ultime_cifre = 'VIP0'
            else:
                pass  # Codice errato o scaduto, procede con il pagamento standard
        ...
        # Calcolo della data di scadenza reale dell'abbonamento
        data_scadenza = (datetime.now() + timedelta(days=giorni_validita)).isoformat()
        
        # --- C. SALVATAGGIO RICEVUTA/TRANSAZIONE NEL DATABASE ---
        nuova_transazione = {
        ...
        # --- D. AGGIORNAMENTO PROFILO UTENTE (Sblocco dell'applicazione) ---
        ...
        # Operazione completata! L'utente ha i permessi di accesso pagati
        session['ha_pagato'] = True
        return redirect(url_for('dashboard'))
...
# ==========================================
# 3. AREA RISERVATA / DASHBOARD DELL'IA
# ==========================================
@app.route('/dashboard')
def dashboard():
    # Sicurezza bloccante 1: Sei loggato?
    if 'utente_loggato' not in session:
        return redirect(url_for('auth'))
    
    # Sicurezza bloccante 2: Hai pagato o usato un codice promo?
...
@app.route('/logout')
def logout():
    session.clear()  # Svuota tutti i dati di sessione per sicurezza
